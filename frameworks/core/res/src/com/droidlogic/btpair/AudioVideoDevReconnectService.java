...
public class AudioVideoDevReconnectService extends Service {
    ...
    class AudioVideoDevPairThread extends Thread {
        ...
        public void run() {
            Looper.prepare();
            Log.i(TAG, "AudioVideoDevPairThread run!!!!");
    ...
    class MyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            Log.i(TAG, "MyHandler.handleMessage:" + msg.what);
            switch (msg.what) {
    ...
    private void connectDevice(String devAddress) {
        Log.e(TAG, "connectDevice:" + devAddress);
        BluetoothDevice device = findDevice(devAddress);
        if (device != null) {
            LocalBluetoothManager localBluetoothManager = getLocalBluetoothManager(mContext);
            if (localBluetoothManager != null) {
                CachedBluetoothDevice cachedDevice = localBluetoothManager.getCachedDeviceManager().findDevice(device);
