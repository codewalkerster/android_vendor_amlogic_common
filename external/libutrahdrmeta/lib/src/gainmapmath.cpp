...
ColorTransformFn getGamutConversionFn(uhdr_color_gamut_t dst_gamut, uhdr_color_gamut_t src_gamut) {
	ALOGE("getGamutConversionFn %d,->%d",src_gamut,dst_gamut);
  switch (dst_gamut) {
    case UHDR_CG_BT_709:
      switch (src_gamut) {
        case UHDR_CG_BT_709:
...
ColorTransformFn getYuvToRgbFn(uhdr_color_gamut_t gamut) {ALOGE("getYuvToRgbFn %d",gamut);
  switch (gamut) {
    case UHDR_CG_BT_709:
      return srgbYuvToRgb;
    case UHDR_CG_DISPLAY_P3:
...
uint32_t colorTo30YUV444(Color e_gamma) {
       // Color pixel= e_gamma/1023.0f;
        Color pixel = bt2100RgbToYuv(e_gamma);
        uint16_t y_uint = (pixel.y * 1023.0f) + 0.5f;

        uint16_t u_uint = (pixel.u * 1023.0f) + 512.0f + 0.5f;
        uint16_t v_uint = (pixel.v * 1023.0f) + 512.0f + 0.5f;
        y_uint = CLIP3(y_uint, 0.0f, 1023.0f);
        u_uint = CLIP3(u_uint, 0.0f, 1023.0f);
        v_uint = CLIP3(v_uint, 0.0f, 1023.0f);
        uint32_t pixelInt = (uint32_t)((uint16_t(v_uint)&0x03ff) | ((uint16_t(u_uint)&0x03ff) << 10)
                | ((uint16_t(y_uint)&0x03ff) << 20) |(0x03<< 30));  // Set alpha to 1.0
        return pixelInt;
}
